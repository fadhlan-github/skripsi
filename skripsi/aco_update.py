...
class PheromoneTable:
    ...
    def deposit(self, path, amount):
        for i in range(len(path)-1):
            self.pheromone[path[i]][path[i+1]] = min(
                MAX_PHEROMONE,
...
def get_path(src, dst, first_port, final_port):
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        if best_paths:
            ...
            for path, _ in best_paths:
                ...
                pheromone_table.deposit(path, Q / total_cost)
    ...
    best_path = max(best_paths, key=lambda x: (x[1] ** ALPHA) * (1 / sum(distances[x[0][i]][x[0][i+1]] for i in range(len(x[0])-1)) ** BETA))[0]
...
class ACOSwitch(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mymacs = {}
        self.mac_to_path = {}
        self.pheromone_table = PheromoneTable()  # Maintain a single pheromone table
        ...
        for s1 in self.switches:
            for s2 in self.switches:
                if adjacency[s1][s2] is not None:
                    ...
                    seed = hash(f"{s1}-{s2}") % 100000
                    random.seed(seed)
                    self.distance_matrix[s1][s2] = random.uniform(1, 10)
