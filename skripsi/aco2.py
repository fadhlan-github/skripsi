...
def get_path(src, dst, first_port, final_port):
    print(f"ACO computing path from DPID: {src} port= {first_port} to DPID: {dst} port {final_port}")
    ...
    for iteration in range(MAX_ITERATIONS):
        for ant in range(N_ANTS):
            ...
            if path[-1] == dst and path_length < best_length:
                best_path = path
                best_length = path_length
                
        if best_path:
            pheromone_table.deposit(best_path, Q / best_length)
        ...
        for src_mac, dst_mac in affected_paths:
            ...
            if src_mac in mymacs and dst_mac in mymacs:
                ...
                if new_path:
                    ...
                    print(f"Installed new path for {src_mac} -> {dst_mac}: {new_path}")
                else:
                    print(f"No alternative path found for {src_mac} -> {dst_mac}")
